...
const LAST_GAME_KEY = 'scorer:lastGame'; const NAMES_KEY = 'scorer:names'; const DEALER_RIG_KEY = 'fivecrowns:dealerRig';
...
function loadDealerRigSettings() {
  try { return normaliseDealerRigSettings(JSON.parse(localStorage.getItem(DEALER_RIG_KEY))); }
  catch (_) { return { ...DEFAULT_DEALER_RIG_SETTINGS }; }
}

function saveDealerRigSettings(settings) {
  const value = normaliseDealerRigSettings(settings);
  try { localStorage.setItem(DEALER_RIG_KEY, JSON.stringify(value)); } catch (_) { /* unavailable */ }
  return value;
}

export {
  loadGame, saveGame, recalledNames, nextRecalledName, lastGameId, hasStartedSave,
