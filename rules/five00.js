...
function suitContractValue(suitId, level) {
  ...
  return suit
    ? (level - FIVE00_RULES.minBid) * FIVE00_RULES.levelStep +
        FIVE00_RULES.baseBid +
        suit.index * FIVE00_RULES.suitStep
    : 0;
}
...
const five00 = {
  ...
  unitLabel: 'sides',
  winDirection: 'high',
  target: FIVE00_RULES.target,
  loseAt: FIVE00_RULES.loseAt,
  onBoardMin: null,
  rounds: OPEN_ROUNDS,
  entry: 'hand',
  allowNegative: true,
    ...
    if (SPECIAL_BIDS[bid.kind]) {
      ...
      players.forEach((player) => {
        if (player.id !== bidderId)
          deltas[player.id] = FIVE00_RULES.defenderTrick * (tricks[player.id] || 0);
      });
    }
  ...
  resolve(players, state) {
    ...
    for (let index = 0; index < hands.length; index++) {
      ...
      if (hand.made && running[hand.bidderId] >= FIVE00_RULES.target)
        ...
        const outs = players
          .filter((player) => running[player.id] <= FIVE00_RULES.loseAt)
    ...
    if (terminal && terminal.type === 'out') {
      const survivors = players
        .filter((player) => !terminal.outIds.includes(player.id))
        .map((player) => player.id);
      const winners = survivors.length ? survivors : leaders;
      const outText = terminal.outIds
        .map((id) => joinNames(players, [id]) + ' out at ' + totals[id])
        .join(', ');
      return {
        totals,
        status: {
...
five00.hand = {
  _blankTricks(players) {
    return objectFromEntries(players.map((player) => [player.id, 0]));
  },
  ...
  newDraft(players) {
    return { bidderId: players[0].id, bid: null, tricks: this._blankTricks(players) };
  },
  ...
  _sum(draft, players) {
    return players.reduce((sum, player) => sum + (draft.tricks[player.id] || 0), 0);
  },
  build(container, draft, players, ui, onChange) {
    ...
    const step = (label, value, onStep) =>
      append(
        labeledStepper(label, value, 0, FIVE00_RULES.tricks, (next) => {
          onStep(next);
          onChange();
        }),
      );
        ...
        return chip(
          ...
          () => {
            ...
            setBid({
              kind,
              suit: suit.id,
              level: (draft.bid && draft.bid.level) || FIVE00_RULES.minBid,
            });
          },
          suit.name,
        );
    ...
    chips(
      Object.entries(SPECIAL_BIDS).map(([kind, special]) =>
        chip(
          special.label,
          draft.bid && draft.bid.kind === kind,
          () => setBid({ kind }),
          special.label,
        ),
      ),
    ...
    if (draft.bid && !SPECIAL_BIDS[draft.bid.kind])
      append(
        labeledStepper(
          'Tricks bid',
          draft.bid.level,
    ...
    if (SPECIAL_BIDS[draft.bid.kind]) {
      ...
      append(
        el(
          'p',
          { class: 'hand-hint' },
          names[opponent.id] + ': ' + otherTricks + ' trick' + (otherTricks === 1 ? '' : 's'),
        ),
      );
    } else
      players.forEach((player) =>
        step(names[player.id], draft.tricks[player.id] || 0, (value) => {
          draft.tricks[player.id] = value;
        }),
      );
  },
  validate(draft, players) {
    if (!draft.bidderId || !draft.bid)
      return { valid: false, message: 'Choose a bidder and contract.' };
    ...
    if (!SPECIAL_BIDS[kind]) {
      ...
      if (sum !== FIVE00_RULES.tricks) {
        return {
          valid: false,
          message: 'Tricks must total ' + FIVE00_RULES.tricks + ' (currently ' + sum + ').',
        };
      }
    }
    const result = five00.scoreHand(
      { bidderId: draft.bidderId, bid: draft.bid, tricks: draft.tricks },
      players,
    );
    ...
    return {
      valid: true,
      message: (result.meta.made ? 'Made' : 'Set') + ' \u00b7 ' + parts.join(' \u00b7 '),
    };
  },
