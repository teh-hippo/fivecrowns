...
const greed = {
  ...
  resolve(players, state) {
    ...
    const finalRound = reached.length
      ? Math.min(...reached) + GREED_FINAL_ROUNDS_AFTER_TARGET
      : null;
    const totals = objectFromEntries(
      players.map((player) => {
    ...
    return {
      ...
      status: {
        ...
        text:
          ...
          ' reached ' +
          GREED_TARGET +
          ' \u2014 one final round, then highest wins',
      },
    };
  },
};
