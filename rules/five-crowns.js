...
const FIVE_CROWNS_ROUNDS = FIVE_CROWNS_WILDS.length;
...
function fiveCrownsSuperRandomCardOrder(random = Math.random) {
...
function validOrder(order, expected) {
  ...
  if (new Set(order).size !== expected.length || !order.every((v) => allowed.has(v)))
...
function fiveCrownsDealerOrder(players, firstDealerIndex = 0, preferredOrder) {
  ...
  const start = Math.max(0, Math.min(ids.length - 1, Math.floor(firstDealerIndex) || 0));
...
function fiveCrownsDealerId(i, state) {
  ...
  if (allowed.has(scheduled)) return scheduled;
  const order = fiveCrownsDealerOrder(players, 0, state.dealerOrder);
  return order.length ? order[i % order.length] : null;
}
function fiveCrownsDealerName(i, state) {
  const id = fiveCrownsDealerId(i, state);
  const player =
...
function fiveCrownsRevealedCount(state) {
  const raw =
    state && typeof state.revealedCount === 'number' && Number.isFinite(state.revealedCount)
      ? state.revealedCount
      : 0;
...
function fiveCrownsAddDealer(state, id, preferenceFor) {
  ...
  const preferred = Array.isArray(state.dealerOrder)
    ? state.dealerOrder.filter((playerId) => playerId !== id)
    : [];
  const oldOrder = fiveCrownsDealerOrder(
    existingIds.map((playerId) => ({ id: playerId })),
    0,
    preferred,
  );
  if (!oldOrder.length) {
  ...
  const currentRounds =
    Array.isArray(state.dealerRounds) && state.dealerRounds.length === FIVE_CROWNS_ROUNDS
      ? state.dealerRounds.slice()
      : fiveCrownsDealerRounds(oldOrder);
  const revealed = fiveCrownsRevealedCount(state);
  const pendingStart = Math.max(0, Math.floor(state.dealerOrderStartsAt) || 0);
  let boundary;
  if (revealed < pendingStart) {
    const order = oldOrder.concat(id);
    const rounds = currentRounds.slice(0, pendingStart);
    for (let i = pendingStart; i < FIVE_CROWNS_ROUNDS; i++)
  ...
  if (revealed === 0) boundary = Math.min(FIVE_CROWNS_ROUNDS, oldOrder.length);
    ...
    const lastDealer = currentRounds[revealed - 1];
    const lastIndex = Math.max(0, oldOrder.indexOf(lastDealer));
    const nextIndex = (lastIndex + 1) % oldOrder.length;
    boundary =
      nextIndex === 0
        ? revealed
        : Math.min(FIVE_CROWNS_ROUNDS, revealed + oldOrder.length - nextIndex);
  }
  const order = oldOrder.concat(id);
  const rounds = currentRounds.slice(0, boundary);
  for (let i = boundary; i < FIVE_CROWNS_ROUNDS; i++)
    rounds[i] = order[(i - boundary) % order.length];
  state.dealerOrder = order;
  state.dealerRounds = rounds;
...
const fiveCrowns = {
  ...
  initVariant(variant, random = Math.random, context = {}) {
    ...
    if (v === 'super-random') {
      ...
      const names = dealerRounds.map((id) => {
  ...
  revealItems(state) {
    ...
    return wilds.map((wild, i) => {
      if (!superRandom)
        return { reels: [{ label: 'Wild', value: wild }], result: wild + ' is wild!' };
      const cardCount = fiveCrownsCardCount(i, state);
      const cards = cardCountText(cardCount);
      return {
        reels: [
          { label: 'Cards', value: String(cardCount), tone: 'cards' },
  ...
  roundLabel(i, state) {
    ...
    if (!state || !fiveCrownsRevealVariant(state.variant) || i < fiveCrownsRevealedCount(state))
      return label;
